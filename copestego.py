...
class SteganographyApp(wx.Frame):
    def __init__(self):
        super().__init__(parent=None, title='Steganography', size=(500, 300))
        panel = wx.Panel(self)
    ...
    def on_encode(self, event):
        ...
        try:
            ...
            temp = wx.Image(image_name, wx.BITMAP_TYPE_ANY)
            '''
            if temp.GetWidth() > 300:
                ratio = temp.GetWidth()/300
                len = int(temp.GetWidth()/ratio)
                hei = int(temp.GetHeight()/ratio)
            else:
                len = int(temp.GetWidth())
                hei = int(temp.GetHeight())
            temp2 = wx.ImageFromBitmap(temp)
            temp2 = temp2.Scale(len, hei, wx.IMAGE_QUALITY_HIGH)
            png = wx.ImageFromBitmap(temp2)
            wx.StaticBitmap(self, -1, png, (40, 180), (png.GetWidth(), png.GetHeight()), size=())
            '''
